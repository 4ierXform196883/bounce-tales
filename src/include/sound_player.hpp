...
public:
    ...
    virtual void playRandomly(const std::string &name, float intervalMin, float intervalMax = 0);
    virtual void stopPlayingRandomly(const std::string &name);
    inline virtual sf::Sound &getSound(const std::string &name) { return sounds.at(name); }
    inline virtual const sf::Sound &getSound(const std::string &name) const { return sounds.at(name); }
