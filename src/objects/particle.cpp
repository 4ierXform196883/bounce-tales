...
void Particle::spawnCircle(const std::string &textureName, const std::string &animationName, const sf::Vector2f &pos, float speed, float radius, size_t amount)
{
    for (size_t i = 0; i < amount; ++i)
    {
        float phi = i * 2 * M_PI / amount;
        sf::Vector2f circlePoint = {radius * std::cos(phi), radius * std::sin(phi)};
        auto ptr = std::make_shared<Particle>(textureName, animationName, circlePoint, speed);
        ptr->setPosition(pos + circlePoint);
        Game::getObjectManager().addObject(ptr);
    }
}

void Particle::spawnScatter(const std::string &textureName, const std::string &animationName, const sf::Vector2f &pos, float speed, const sf::Vector2f &delta, size_t amount)
{
    for (size_t i = 0; i < amount; ++i)
    {
        float x = randomizer::getRandFloat(pos.x - delta.x, pos.x + delta.x);
        ...
        float dx = randomizer::getRandFloat(-1, 1);
        float dy = randomizer::getRandFloat(-1, 1);
        auto ptr = std::make_shared<Particle>(textureName, animationName, sf::Vector2f(dx, dy), speed);
        ptr->setPosition(x, y);
        Game::getObjectManager().addObject(ptr);
    }
}

void Particle::spawnScatterDirected(const std::string &textureName, const std::string &animationName, const sf::Vector2f &pos, float speed, const sf::Vector2f &delta, const sf::Vector2f &dir, size_t amount)
{
    for (size_t i = 0; i < amount; ++i)
    {
        float x = randomizer::getRandFloat(pos.x - delta.x, pos.x + delta.x);
        float y = randomizer::getRandFloat(pos.y - delta.y, pos.y + delta.y);
        auto ptr = std::make_shared<Particle>(textureName, animationName, dir, speed);
        ptr->setPosition(x, y);
        Game::getObjectManager().addObject(ptr);
    }
}

Particle::Particle(const std::string &textureName, const std::string &animationName, const sf::Vector2f &dir, float speed)
    : GameObject("particle"), dir(normalized(dir)), speed(speed), isAnimation(true)
{
    ...
    drawable = std::make_shared<Animation>(texture, animationBounds, 5 * speed);
