...
void ObjectManager::load(const std::string &path)
{
    ...
    camera = std::make_shared<Camera>(sf::Vector2f(0, 0), Game::getSettings().camera_size);
    background = std::make_shared<Background>(30, 20);
    player = std::make_shared<Player>(sf::Vector2f(500, -500), 0.1f, 10.0f, 0.05f, 0.2f);
    ...
    auto ptr = std::make_shared<JumpPad>(sf::Vector2f(750, 320), 15);
...
{
    for (auto grd : ground)
        GameObject::calculateCollision(player, grd);
    for (auto cld : collidables)
        GameObject::calculateCollision(player, cld);
}
...
void ObjectManager::drawAll(sf::RenderTarget &target)
{
    ...
    for (auto grd : ground)
        GameObject::draw(grd, target);
    for (auto obj : objects)
        GameObject::draw(obj, target);
