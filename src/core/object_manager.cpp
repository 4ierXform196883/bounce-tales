...
void ObjectManager::load(const std::string &path)
{
    ...
    {
        verts.push_back({(float)polygon[0][indices[i]][0], (float)polygon[0][indices[i]][1] - 600});
    }
...
void ObjectManager::calculateCollision(std::shared_ptr<GameObject> first, std::shared_ptr<GameObject> second)
{
    ...
    case 2: // concave
    {
        auto physical = first->getPhysical();
        sf::Vector2f penetration = {0, 0};
        // sf::Vector2f penetration = {1, 1};
        int cnt = 0;
        ...
        penetration = {-penetration.x / cnt, -penetration.y / cnt};
