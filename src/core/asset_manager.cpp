...
std::map<std::string, bool> repeated;
std::map<std::string, bool> smooth;
...
void AssetManager::loadTextures(const std::string &path)
{
  ...
  for (auto &entry : fs::directory_iterator(path))
  {
    ...
    else if (entry.path().extension() == ".png")
    {
      ...
      textures[filename].setRepeated(repeated.find(filename) == repeated.end() ? true : repeated[filename]);
      textures[filename].setSmooth(smooth.find(filename) == smooth.end() ? true : smooth[filename]);
    }
  }
}
